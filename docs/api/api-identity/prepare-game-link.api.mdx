...
This is designed to be as flexible as possible so `link_url` can be opened
on any device. For example, when playing a console game, the user can scan a
QR code for `link_url` to authenticate on their phone.

<div><ApiTabs><TabItem label={"200"} value={"200"}><div>

PrepareGameLink 200 response

</div><div><details style={{}} data-collapsed={false} open={true}><summary style={{"textAlign":"left"}}><strong>Schema</strong></summary><div style={{"textAlign":"left","marginLeft":"1rem"}}></div><ul style={{"marginLeft":"1rem"}}><SchemaItem collapsible={false} name={"identity_link_token"} required={true} schemaDescription={"Pass this to `rivet.api.identity#GetGameLink` to get the linking status.\n\nValid for 15 minutes."} schemaName={"string"} qualifierMessage={undefined}></SchemaItem><SchemaItem collapsible={false} name={"identity_link_url"} required={true} schemaDescription={"The URL that the user should visit to link their Rivet account."} schemaName={"string"} qualifierMessage={undefined}></SchemaItem><SchemaItem collapsible={false} name={"expire_ts"} required={true} schemaDescription={"Timestamp (in milliseconds) at which the link will expire."} schemaName={"int64"} qualifierMessage={undefined}></SchemaItem></ul></details></div></TabItem><TabItem label={"400"} value={"400"}><div>
